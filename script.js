...
const gringottsSummaryReset = document.querySelector(
  ".gringotts__summary--reset"
);
...
const displayBalance = function (sum) {
  const balance = sum.reduce(function (acc, cur) {
    return acc + cur;
  }, 0);
...
L.marker([51.5, -0.09]).addTo(map).bindPopup("Snape").openPopup();
...
const findCountry = async function (country) {
  try {
    ...
    const markup = `
        ...
        <div class="country__name"><h1>${finalData.name.slice(
          0,
          6
        )} →</h1></div>
        <div class="country__city"><span>${finalData.capital.slice(
          0,
          9
        )}</span></div>
     `;
